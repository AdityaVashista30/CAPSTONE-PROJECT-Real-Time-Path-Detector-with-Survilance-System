...
class admin(QMainWindow):
    ...
    def del_user(self):
        ...
        if len(uId)==0 :
            ...
            if temp is None:
                ...
                cur.execute('Delete FROM Users where user= ?',(uId,))
                conn.commit()
                self.label_16.setText("USER "+uId+" DELETED \n SUCCESFULLY")
        cur.close()
        self.show_users()
    ...
    def addUser(self):  
        ...
        if len(userIn)==0 or len(passIn)==0 or len(nameIn)==0 or typeIn=="SELECT":
    ...
    def stop_cam(self):
        if self.camOn:
            ...
            self.camOn=False
            self.timer.stop()
            img=cv2.imread("D:/Projects/CAPSTONE/reset.PNG")
            qformat = QImage.Format_RGB888
